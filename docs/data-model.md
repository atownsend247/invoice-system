...
sixteen entries: the flattened baseline (2026-09-16), migration 2 (added `tax_rate`
...
and migration 16 (added `registrars` — another brand new table plus its
own `idx_registrars_organisation` index, no rebuild needed either — see
the `Registrar` row below). Schema changes from here on are new entries
appended to that list, not edits to any of these sixteen.
