...
test('saving business details persists them across a reload', async ({ authenticatedPage: page }) => {
  await page.goto('/settings')

  await page.getByLabel('Business name').fill('Contoso Consulting')
  await page.getByLabel('Business address').fill('1 Market Street, London')
  ...
  await expect(page.getByLabel('Business address')).toHaveValue('1 Market Street, London')
...
test('UTR and VAT number can be left blank', async ({ authenticatedPage: page }) => {
  await page.goto('/settings')

  await page.getByLabel('Business name').fill('Sole Trader Co')
  await page.getByLabel('Business address').fill('2 Market Street, London')
